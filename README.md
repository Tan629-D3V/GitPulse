...
1. **Dashboard "Live Predictions" section isn't wired up.** `models/src/infer.py` runs
   correctly standalone and prints ranked abandonment risk scores, but the dashboard doesn't
   yet call it or render those scores in the UI. This is the single most valuable remaining
   task — it's what makes the full loop (stream -> feature -> predict -> display) visible to a
   viewer instead of requiring a terminal command.

2. **Airflow DAG doesn't exist.** The Airflow container runs as part of the stack, but no DAG
   orchestrates the batch job — it's still triggered manually via `spark-submit`. Provisioned
   for future scheduling, not implemented.

3. **No cloud deployment.** Everything runs locally via Docker Compose. Databricks Community
   Edition migration (for the batch job) and Neon (managed cloud Postgres) were planned but
   not started. Streamlit Community Cloud deployment for a public dashboard URL also not done.

4. **Batch data is 60 days, not the originally planned 6 months.** A deliberate scope
   decision for the deadline — 60 days is enough for real week-over-week trend signal, but a
   production version would want more history for stronger seasonal/trend detection.

5. **Only 263 of 455 curated repos have batch feature history.** The other repos had no
   matching activity in the sampled GHArchive hours — expected given a 60-day/1-hour-per-day
   sample rather than a full historical pull.

6. **Model precision is modest (0.577 on test).** Recall was deliberately prioritized over
   precision for the early-warning use case, but the model should be understood as a proof of
   concept validating the pipeline, not a production-grade risk scorer. More training data and
   feature iteration would be needed before real-world use.

7. **Deployment screenshots are placeholders in the presentation deck** — need to be replaced
   with actual dashboard and Spark UI screenshots before presenting.

8. **Intermittent SSL certificate verification errors** on some GitHub API polls from within
   the producer container (`self-signed certificate in certificate chain`) — doesn't block the
   pipeline (still successfully sends the large majority of events per cycle) but is an
   unresolved environment issue, likely related to network-level TLS interception (antivirus
   or corporate network inspection), not yet root-caused.

9. **XGBoost was the only model actually trained.** CatBoost and LightGBM were considered as
   alternatives during design (see stack rationale) but never implemented — worth being
   precise about this if asked, since an earlier architecture diagram listed all three as if
   used.

10. **No SHAP explainability layer, no Prophet virality forecasting.** Both were part of the
    original broader project vision but deliberately deferred — planned as a separate, future
    project rather than folded into GitPulse under deadline pressure.

11. **WSL2 resource constraints remain a real operating limitation.** The full stack requires
    the `.wslconfig` memory cap workaround to run reliably on 8GB RAM; this is a development-
    environment tradeoff, not something a genuine multi-machine deployment would need to work
    around.
